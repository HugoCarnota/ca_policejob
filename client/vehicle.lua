...
print(type)
					...
					if #shopElements > 0 then
						OpenShopMenu(shopElements, playerCoords, shopCoords,type)
					else
				...
				if #jobVehicles > 0 then
					...
					for k,v in ipairs(jobVehicles) do
						...
						if IsModelInCdimage(props.model) then
							...
							if v.stored then
								label = label .. ('<span style="color:white;">%s</span>'):format(_U('garage_stored'))
							else
								label = label .. ('<span style="color:white;">%s</span>'):format(_U('garage_notstored'))
							end
					...
					table.insert(garage, {label = "Llamar a la grúa", value = "grua"})
					...
					if #garage > 0 then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
							...
							if data2.current.stored then
								...
								-- print(foundSpawn)
								-- print(spawnPoint.coords, spawnPoint.heading)
								-- print(station, part, partNum)
								if foundSpawn then
									menu2.close()
									...
									ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
										...
										local vehicleLabel = GetDisplayNameFromVehicleModel(vehicleProps.model)

										SetEntityAsMissionEntity(vehicle, true, true) --this is mandatory in order to retrieve the network ID
							...
							elseif data2.current.value == "grua" then
								menu2.close()
								menu.close()
								for k,v in pairs(data2.elements) do
									
									if v.stored == false then 
										--print(v.stored, v.plate)
										TriggerServerEvent('eden_garage:modifystate', v.plate, true)
									end
									Citizen.Wait(100)
								end
								ESX.ShowNotification("La grúa ha devuelto los coches al garaje")
...
function OpenShopMenu(elements, restoreCoords, shopCoords,type)
